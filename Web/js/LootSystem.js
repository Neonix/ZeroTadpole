...
(function() {
    ...
    class LootManager {
        ...
        collectLoot(dropId, playerId) {
            ...
            // Server authoritative inventory: ask server to pick up, don't modify inventory locally
            if (window.GameSystems && window.GameSystems.inventory && window.GameSystems.inventory._serverMode && window.webSocketService && window.webSocketService.sendLootPickup) {
                const now = Date.now();
                this._pickupLast = this._pickupLast || {};
                const sid = drop.serverId || drop.id;
                const last = this._pickupLast[sid] || 0;
                if (now - last > 800) {
                    this._pickupLast[sid] = now;
                    window.webSocketService.sendLootPickup(sid);
                }
                // keep local drop until server confirms via loot_pickup
                return item;
            }
