var WebSocketService = function(model, webSocket, reconnectFn) {
	...
	// Toast throttle helper to prevent UI spam (case-insensitive + per message + per slot)
	var normalizeToastKey = function(input) {
		try {
			return String(input || '')
				.trim()
				.toLowerCase()
	...
	var toastOncePer = function(key, message, type, ms, opts) {
		if (!window.showToast) return;
		webSocketService._toastLast = webSocketService._toastLast || {};
		var now = Date.now();
		opts = opts || {};
		var slot = (typeof opts.slot === 'number') ? opts.slot : null;
		var normKey = normalizeToastKey(key);
		var finalKey = normKey + '|slot:' + (slot !== null ? slot : 'any');
		var last = webSocketService._toastLast[finalKey] || 0;
		if ((now - last) < ms) return;
		webSocketService._toastLast[finalKey] = now;
		window.showToast(message, type || 'warning', { slot: slot, throttleMs: ms });
	};
	...
	this.world_snapshotHandler = function(data) {
		...
		if (data.players && Array.isArray(data.players) && model) {
			...
			data.players.forEach(function(playerData) {
				if (!playerData || playerData.i === undefined || playerData.i === null) return;
				// Ne pas mettre à jour notre propre position
				if (model.userTadpole && String(playerData.i) === String(model.userTadpole.id)) return;
				seenPlayers[String(playerData.i)] = true;
				
				// IMPORTANT: créer le têtard si on ne l'a jamais vu (sinon "plus d'autres joueurs")
				var tadpole = model.tadpoles[playerData.i];
				if (!tadpole) {
					tadpole = ensureTadpole(playerData.i);
					// spawn direct
					tadpole.x = playerData.x;
					tadpole.y = playerData.y;
				}

				// Nom/couleur dans le snapshot (n/c)
				if (playerData.n) tadpole.name = playerData.n;
				if (playerData.c) tadpole.color = playerData.c;
			...
			// Nettoyer les joueurs qui ne sont plus dans le snapshot
			try {
				var selfId = model.userTadpole ? String(model.userTadpole.id) : null;
				Object.keys(model.tadpoles).forEach(function(id) {
					if (id === selfId || id === 'npc-guide') return;
					var tp = model.tadpoles[id];
					if (tp && tp.isNpc) return;
					if (!seenPlayers[id]) {
						delete model.tadpoles[id];
						delete model.arrows[id];
					}
	...
	this.mob_attackHandler = function(data) {
		...
		if (data.targetId && model.userTadpole && String(data.targetId) === String(model.userTadpole.id)) {
			try { window.GameSystems.combat.createDamageNumber(model.userTadpole.x, model.userTadpole.y, data.damage); } catch (e) {}
		}
	}
	...
	this.mob_hitHandler = function(data) {
		...
		if (mob) {
			...
			try { combat.createDamageNumber(mob.x, mob.y, data.damage); } catch (e) {}
		}
	}

	// Handler pour attaque melee (server-authoritative)
	this.melee_attackHandler = function(data) {
		if (!data || !window.GameSystems || !window.GameSystems.combat) return;
		var combat = window.GameSystems.combat;
		var mob = combat.mobs.find(function(m) { return m.serverId === data.mobId; });
		if (mob) {
			try {
				combat.createDamageNumber(mob.x, mob.y, data.damage || 0);
				combat.registerSpellImpact && combat.registerSpellImpact(mob.x, mob.y, { icon: '👊' }, mob);
			} catch (e) {}
		}
	};

	// Handler for melee result / cooldown state
	this.melee_resultHandler = function(data) {
		try {
			if (!window.GameSystems) return;
			window.GameSystems.gameState = window.GameSystems.gameState || {};
			if (typeof data.cooldownMs === 'number' && data.cooldownMs > 0) {
				window.GameSystems.gameState.meleeCooldownMs = data.cooldownMs;
			}
			if (typeof data.cooldownUntil === 'number') {
				window.GameSystems.gameState.meleeCooldownUntil = data.cooldownUntil;
			}
			if (data && data.success === false && data.error) {
				// Use toast throttling (already global)
				window.showToast && window.showToast(String(data.error), 'error', { key: 'melee|' + String(data.error).toLowerCase(), throttleMs: 800 });
			}
		} catch (e) {}
	};
	...
	// Death UI
	this.deathHandler = function(data) {
		try {
			window.inputState = window.inputState || {};
			window.inputState.isDead = true;
			window.inputState.isMoving = false;
			window.inputState.useJoystick = false;

			var respawnAt = (data && typeof data.respawnAt === 'number') ? data.respawnAt : (Date.now() + (data && data.respawnIn ? data.respawnIn : 3000));
			window.__respawnAt = respawnAt;
			...
			if (!overlay) {
				...
				overlay.innerHTML = '<div class="death-box"><div class="death-title">💀 Tu es mort</div><div class="death-sub" id="death-sub">Respawn…</div></div>';
				document.body.appendChild(overlay);
			}
			overlay.classList.add('active');

			if (window.__deathInterval) clearInterval(window.__deathInterval);
			window.__deathInterval = setInterval(function() {
				var ms = Math.max(0, (window.__respawnAt || 0) - Date.now());
				var s = Math.ceil(ms / 1000);
				var sub = document.getElementById('death-sub');
				if (sub) sub.textContent = 'Respawn dans ' + s + 's…';
			}, 120);
		} catch (e) {}
	};

	this.respawnHandler = function(data) {
		try {
			window.inputState = window.inputState || {};
			window.inputState.isDead = false;
			var overlay = document.getElementById('death-overlay');
			if (overlay) overlay.classList.remove('active');
			if (window.__deathInterval) clearInterval(window.__deathInterval);
			window.__deathInterval = null;
			window.showToast && window.showToast('✅ Respawn', 'success');
		} catch (e) {}
	};
	...
	this.loot_dropHandler = function(data) {
		// Deprecated client-side loot drop (created fake IDs and caused ghost loot).
		// Server-authoritative loot uses: loot_spawn + loot_remove + loot_pickup.
		// Ignore this message type to avoid desync.
		return;
	}
	...
	this.inventory_syncHandler = function(data) {
			if (window.GameSystems && window.GameSystems.inventory) {
				...
				// Mark server inventory authoritative
				window.GameSystems.inventory.setServerInventory && window.GameSystems.inventory.setServerInventory(data.slots, data.gems, data.gold, data.maxSlots);
				window.GameSystems.inventory.updateUI && window.GameSystems.inventory.updateUI();
				// Refresh profile/bag UI if open
				try {
					var profile = document.getElementById('profile-panel');
					if (profile && !profile.classList.contains('hidden') && window.updateProfilePanel) {
	...
	// Server-authoritative dash/speed state (slot R by default)
	this.boost_stateHandler = function(data) {
		try {
			if (!data || !model || !model.userTadpole) return;
			if (String(data.playerId) !== String(model.userTadpole.id)) {
				return;
			}
			model.boostUntil = (typeof data.until === 'number') ? data.until : 0;
			model.boostMultiplier = (typeof data.multiplier === 'number') ? data.multiplier : 1.8;
			model.boostCooldownUntil = (typeof data.cooldownUntil === 'number') ? data.cooldownUntil : 0;
			var slot = (typeof data.slot === 'number') ? data.slot : 3;
			if (window.GameSystems && window.GameSystems.gameState && typeof model.boostCooldownUntil === 'number') {
				window.GameSystems.gameState['spell_cooldown_' + slot] = model.boostCooldownUntil;
			}
		} catch (e) {}
	};
	...
	this.inventory_resultHandler = function(data) {
		...
		if (!data.result.success) {
			var err = String(data.result.error || 'Erreur');
			var slot = (typeof data.result.slot === 'number') ? data.result.slot : null;
			toastOncePer('inv_err_' + err, '❌ ' + err, 'error', 1200, { slot: slot });
		}
	};
	...
	this.spell_castHandler = function(data) {
			if (!(window.GameSystems && window.GameSystems.combat)) return;
			var combat = window.GameSystems.combat;
			var spell = data ? data.spell : null;
			// Avoid double-casting for the local player (client-side prediction)
			if (spell && model && model.userTadpole && String(spell.casterId) === String(model.userTadpole.id)) {
				var now = Date.now();
				if (combat._lastLocalSpellCastAt && (now - combat._lastLocalSpellCastAt) < 350) {
					if (!combat._lastLocalSpellId || combat._lastLocalSpellId === spell.spellId) {
						return;
					}
				}
			}
			combat.handleRemoteSpell(spell);
		};
	
	this.spell_resultHandler = function(data) {
		if (!data.result) return;
		if (!data.result.success) {
			var err = String(data.result.error || 'Sort en recharge');
			// Throttle cooldown messages (holding key down shouldn't spam)
			var slot = (typeof data.result.slot === 'number') ? data.result.slot : null;
			toastOncePer('spell_err_' + err, '⏳ ' + err, 'warning', 900, { slot: slot });
		}
	};
	...
	this.loot_pickup_failedHandler = function(data) {
			var now = Date.now();
			var reason = (data && data.reason) ? String(data.reason) : 'Impossible de ramasser';
			toastOncePer('loot_fail_' + reason, '❌ ' + reason, 'warning', 1500);
			// If inventory full, temporarily block auto-pickup attempts
			if (window.GameSystems && window.GameSystems.combat) {
				var low = reason.toLowerCase();
				if (low.indexOf('inventaire') !== -1 || low.indexOf('plein') !== -1) {
					window.GameSystems.combat._invFullUntil = now + 2000;
				}
			}
		};
	...
	this.sendMelee = function() {
		webSocket.send(JSON.stringify({type: 'melee'}));
	}
