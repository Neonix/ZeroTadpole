...
class PetSystem
{
    ...
    public function commandSpecial($playerId, $targetX, $targetY)
    {
        if (!isset($this->playerPets[$playerId])) {
            return ['success' => false, 'error' => 'Joueur non initialisé'];
        }
        ...
        // Si aucun pet actif, tenter d'en activer un automatiquement
        if (empty($data['activePet'])) {
            $ownedIds = array_keys($data['ownedPets']);
            if (empty($ownedIds)) {
                return ['success' => false, 'error' => 'Aucun compagnon'];
            }
            $petId = $ownedIds[0];
            $activate = $this->activatePet($playerId, $petId);
            if (empty($activate['success'])) return $activate;
            return ['success' => true, 'activated' => true, 'petId' => $petId];
        }

        $activePetId = $data['activePet'];
        $pet = $data['ownedPets'][$activePetId] ?? null;
        $state = $data['petStates'][$activePetId] ?? null;
        if (!$pet || !$state) {
            return ['success' => false, 'error' => 'Compagnon indisponible'];
        }

        $petType = $this->petTypes[$pet['typeId']] ?? null;
        if (!$petType) return ['success' => false, 'error' => 'Type de compagnon inconnu'];

        $now = microtime(true) * 1000;
        $cd = (int)($petType['abilityCD'] ?? 2000);
        $last = (float)($state['lastAbility'] ?? 0);
        $remaining = max(0, $cd - ($now - $last));
        if ($remaining > 0) {
            return ['success' => false, 'error' => 'Compagnon en recharge', 'cooldown' => $remaining];
        }

        // Trouver une cible près du point visé, sinon autour du joueur
        $target = $this->findMobNearPoint((float)$targetX, (float)$targetY, 220);
        if (!$target) {
            $player = $this->worldServer->getEntityById($playerId);
            if ($player) {
                $target = $this->findPetTarget($playerId, $player->x, $player->y);
        ...
        return ['success' => true, 'petId' => $activePetId, 'targetId' => $target['id']];
    }

    private function findMobNearPoint($x, $y, $radius)
    {
        $nearest = null;
        $nearestDist = (float)$radius;
