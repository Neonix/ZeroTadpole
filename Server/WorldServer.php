...
class WorldServer 
{
    ...
    public function tryMobAttack(&$mob, $player)
    {
        ...
        if ($player->hitPoints <= 0) { $player->hitPoints = 0; $player->isDead = true; }
        
        $this->pushToPlayer($player, ['type' => 'health', 'tick' => $this->serverTick, 'time' => $this->getServerTime(), 'points' => $player->hitPoints]);
    ...
    public function generateMobLoot($mob, $killer)
    {
        ...
        if (!empty($drops)) {
            foreach ($drops as $itemType) {
                $lootId = 'loot-' . $this->lootNextId++;
                $this->lootItems[$lootId] = [
                    'id' => $lootId, 'itemType' => $itemType,
                    'x' => $mob['x'] + (mt_rand() / mt_getrandmax() - 0.5) * 20,
                    'y' => $mob['y'] + (mt_rand() / mt_getrandmax() - 0.5) * 20,
                    'spawnedAt' => microtime(true), 'expiresAt' => microtime(true) + 60,
                    'killerId' => $killer ? $killer->id : null
                ];
            }
            $this->pushBroadcast(['type' => 'loot_drop', 'tick' => $this->serverTick, 'time' => $this->getServerTime(), 'x' => $mob['x'], 'y' => $mob['y'], 'items' => $drops, 'killerId' => $killer ? $killer->id : null]);
        }
    }
    ...
    public function handleLootPickup($player, $lootId)
    {
        ...
        if (!$addResult['success']) {
            $this->pushToPlayer($player, ['type' => 'loot_pickup_failed', 'reason' => $addResult['error'] ?? 'Inventaire plein']);
            return false;
        }
    ...
    public function createLoot($itemType, $x, $y, $expiresIn = 60)
    {
